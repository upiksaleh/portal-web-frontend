...
export class ApplicationsResource extends BaseResourceClass<'applications'> {
  ...
  config() {
    this.query = {
      fields: [
        'id', 'link', 'sort', 'title', 'links', 'social_media', 'description', 'slug', 'logo.*', 'categories.id',
        'categories.category.id', 'categories.category.name', 'organization.id', 'organization.name', 'organization.slug'
      ]
    }
  }
  ...
  protected apiByPaths(): Record<string, any> {
    return {
      byCategories: () => {
        ...
        this.andFilter({categories: {category: {id: {_in: categories}}}})
      }
    }
  }
