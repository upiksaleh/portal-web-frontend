...
export type UISwrComponentProps = {
  ...
  wrapperComponent?: ({children, ...data}: { children: any } & SWRResponse) => ReactElement,
  swrData: SWRResponse,
...
export const UISwrComponent: FC<UISwrComponentProps> = (
  ...
  if (!swrData.data.data || (swrData.data.data && !swrData.data.data.length)) return wrapper(noItemsComponent(swrData));
